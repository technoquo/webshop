<div class="grid grid-cols-4 mt-12 gap-4">
    ...
    <div>
        <div class="bg-white rounded-lg shadow p-5 col-span-1">
            @guest
                <p><a href="{{ route('register') }}" class="underline">Regístrese</a> o <a href="{{ route('login') }}" class="underline">inicie sesión</a> para
                    continuar.</p>
            @endguest
    
            @auth
                <x-button wire:click="checkout">{{ __('Pedido') }}</x-button>
            @endauth
        </div>
    </div>
</div>
